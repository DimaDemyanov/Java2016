...
syms x1;
syms x2 x3 x4 x5;
z = (-2*x1+x2+2)/(x1+3*x2+4);   % Символьная целевая функция
N = [-1 1;
     0 1;
     2 1];
B = eye(3);
...
m = sizeA(1,1);
Indexes = zeros(1, m);
Indexes = [3, 4, 5]; % Индексы базисных векторов
n = sizeA(1,2);
x = A\b;
...
x(1)    % Вывод на экран полученную оптимальную точку
x(2)